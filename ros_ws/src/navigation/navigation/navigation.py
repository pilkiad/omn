...
class Navigation(LifecycleNode):
    ...
    def goal_pose_callback(self, msg):
        ...
        # Ignore waypoints that are almost identical to the last accepted one.
        if self.last_received_goal is not None:
            ...
            if spacing < self.MIN_GOAL_SPACING:
                self.get_logger().debug(
                    f'Ignoring duplicate goal; spacing={spacing:.3f}m'
                )
                return
        ...
        # Do not replace the current goal. Add later goals in FIFO order.
        if self.has_goal:
    ...
    def activate_next_goal(self):
        ...
        self.goal_x = next_goal[0]
        self.goal_y = next_goal[1]
        self.has_goal = True
        self.last_goal_time = self.now_seconds()
        self.clear_planned_path(reset_planning_context=True)
        self.get_logger().info(
            f'Activated next queued goal x={self.goal_x:.3f}, '
            f'y={self.goal_y:.3f}; '
            f'remaining={len(self.goal_queue)}'
        )
        return True
    ...
    def timer_callback(self):
        ...
        if (
            self.distance(self.x, self.y, self.goal_x, self.goal_y)
            <= self.GOAL_TOLERANCE
            and self.goal_queue
        ):
            self.get_logger().info(
                f'Waypoint reached ({self.goal_x:.2f},{self.goal_y:.2f}) '
