...
#            planned_path_grid = self.theta_star(start, goal)
    ...
    def theta_star(self, start, goal):
        ...
        self.get_logger().info(
            f"[Theta*] START planning start={start} goal={goal}"
        )

        if not self.is_raw_free(start):
            self.get_logger().error(
                f"[Theta*] FAILED: start {start} is not free"
            )
            return []

        if not self.is_raw_free(goal):
            self.get_logger().error(
                f"[Theta*] FAILED: goal {goal} is not free"
            )
            return []
        ...
        while open_set:
            ...
            self.get_logger().info(
                ...
                f"cell={current} "
                f"f={f_current:.3f} "
                f"g={g_score.get(current, math.inf):.3f} "
                f"open={len(open_set)} "
                f"closed={len(closed)}"
            )
            ...
            if current in closed:
                self.get_logger().info(
                    f"[Theta*] SKIP {current} already closed"
                )
                continue

            if current == goal:
                path = self.reconstruct_path_theta(
                    came_from,
                    current
                )
                self.get_logger().info(
                    ...
                    f"path_cells={len(path)} "
                    f"shortcuts={shortcuts} "
                    f"normal_updates={normal_updates}"
                )
                return path
            ...
            self.get_logger().info(
                f"[Theta*] Parent of {current} is {parent}"
            )

            for neighbor, step_cost in self.neighbors(current, goal):
                if neighbor in closed:
                    self.get_logger().info(
                        f"[Theta*] IGNORE neighbor={neighbor} already closed"
                    )
                    continue

                if parent is not None:
                    visible = self.line_of_sight(
                        parent,
                        neighbor
                    )
                    self.get_logger().info(
                        f"[Theta*] LOS {parent}->{neighbor} = {visible}"
                    )
                    ...
                    if visible:
                        tentative = (
                            ...
                            + self.inflation_penalty(neighbor)
                        )
                        old = g_score.get(
                            neighbor,
                            math.inf
    ...
    def reconstruct_path_theta(self, came_from, current):
        path = [current]
