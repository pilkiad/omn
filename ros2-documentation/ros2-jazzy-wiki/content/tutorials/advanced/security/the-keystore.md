...
docname: "Tutorials/Advanced/Security/The-Keystore"
...
<a id="understanding-the-security-keystore"></a>
<a id="the-keystore"></a>
...
- [Security Artifact Locations](#security-artifact-locations)

  - [Public Key Materials](#public-key-materials)
  - [Private Key Materials](#private-key-materials)
  - [Domain Governance Policy](#domain-governance-policy)
  - [Security Enclaves](#security-enclaves)
...
<a id="security-artifact-locations"></a>
...
In a public key infrastructure, the [Certificate Authority](https://en.wikipedia.org/wiki/Certificate_authority) acts as a trust anchor: it validates the identities and permissions of participants.
...
:   - The certificate subject name `sros2CA` is the default provided by the `sros2` utilities.
    ...
    - As a Root Certificate Authority, this is a [self-signed certificate](https://en.wikipedia.org/wiki/Self-signed_certificate); i.e., it is signed using its own private key.

Since this is a public certificate, it can be freely copied as needed to establish trust throughout your ROS system.

<a id="private-key-materials"></a>
...
The `sros2` utilities use [elliptic curve cryptograpy](https://en.wikipedia.org/wiki/Elliptic-curve_cryptography) rather than RSA for improved security and reduced key size.
...
<a id="domain-governance-policy"></a>
...
Use the following command to validate the [S/MIME signature](https://en.wikipedia.org/wiki/S/MIME) of the governance file:
...
For more details about security enclaves, see the [design document](https://design.ros2.org/articles/ros2_security_enclaves.html).
...
Each file **must** be named as defined below, and as outlined in the [DDS Security standard](https://www.omg.org/spec/DDS-SECURITY/1.1/About-DDS-SECURITY/).
